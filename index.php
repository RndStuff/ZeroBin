...
function dataid2path($dataid)
{
    ...
    return $_config['data_dir'] . '/' . substr($dataid, 0, 2) . '/' . substr($dataid, 2, 2) . '/';
}
...
function deletePaste($pasteid)
{
    ...
    unlink($path . $pasteid);
    unlink($path . $pasteid . $_config['salt_append']);
...
if (!empty($_POST['data'])) // Create new paste/comment
{
    ...
    */
    ...
    if ($_config['paste_id_len'] <= 32) {
        $dataid = str_shuffle(substr(hash('md5', $data), 0, $_config['paste_id_len']));
    } else {
        $dataid = str_shuffle(substr(md5($data) . generateRandomString($_config['paste_id_len'], true), 0, $_config['paste_id_len']));
    }
...
$page->assign('VERSION', $_config['version']);
